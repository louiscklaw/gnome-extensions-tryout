...
async function refreshWeatherData() {
  ...
  try {
    json = await this.loadJsonAsync(owmCurrentUrl, params).then(async json => {
      try {
...
async function refreshForecastData() {
  ...
  try {
    json = await this.loadJsonAsync(owmForecastUrl, params).then(async json => {
      processing: try {
        ...
        // Today's forecast
        todayList = await this.processTodaysData(json).then(async todayList => {
          try {
        ...
        sortedList = await this.processForecastData(json).then(async sortedList => {
          try {
...
function populateCurrentUI() {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (this._translate_condition && !this._providerTranslations) comment = getWeatherCondition(json.weather[0].id);
      ...
      if (json.wind != undefined && json.wind.deg != undefined) {
        ...
        if (json.wind.gust != undefined) this._currentWeatherWindGusts.text = this.formatWind(json.wind.gust);
      } else {
