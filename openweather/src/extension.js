...
let OpenWeatherMenuButton = GObject.registerClass(
  class OpenWeatherMenuButton extends PanelMenu.Button {
      ...
      this._settingsC = this._settings.connect('changed', () => {
        if (this.disableForecastChanged()) {
          ...
          if (this.menuAlignmentChanged()) {
            if (this._timeoutMenuAlignent) GLib.source_remove(this._timeoutMenuAlignent);
    ...
    get _clockFormat() {
      if (!this._settingsInterface) this.loadConfigInterface();
    ...
    get _appid() {
      ...
      else key = this._settings.get_string('appid');
    ...
    extractCoord() {
      ...
      if (arguments[0] && arguments[0].search('>') != -1) coords = arguments[0].split('>')[0].replace(' ', '');
    ...
    toBeaufort(w, t) {
      ...
      else if ((w > 5, 4 && w <= 7.9)) return !t ? '4' : '(' + _('Moderate breeze') + ')';
    ...
    checkAlignment() {
      ...
      if (Clutter.get_default_text_direction() == Clutter.TextDirection.RTL) menuAlignment = 1.0 - menuAlignment;
    ...
    formatWind(speed, direction) {
      ...
      if (!speed) return '\u2013';
      else if (speed === 0 || !direction) return parseFloat(speed).toLocaleString(this.locale) + ' ' + unit;
      // i.e. speed > 0 && direction
      else return direction + ' ' + parseFloat(speed).toLocaleString(this.locale) + ' ' + unit;
    }
    ...
    rebuildFutureWeatherUi(cnt) {
      ...
      pan.connect('pan', action => {
        ...
        this.scrollForecastBy(
          -1 * (dx / this._forecastScrollBox.width) * this._forecastScrollBox.hscroll.adjustment.page_size,
        );
        return false;
      });
