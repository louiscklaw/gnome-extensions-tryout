...
const { Adw, Gtk, GObject, Soup, GLib } = imports.gi;
  ...
  class OpenWeather_SearchResultsWindow extends Adw.PreferencesWindow {
    ...
    async _findLocation() {
      ...
      // MapQuest
        ...
        if (_mqKey === '') {
          ...
          this.resultsStatus.set_description(
            _('You need an AppKey to use MapQuest, get one at: %s').format('developer.mapquest.com'),
          );
      ...
      // Geocode.Farm
        ...
        try {
          json = await this._loadJsonAsync(_gcodeUrl, params).then(async json => {
            if (!json) {
              this._resultsError(true);
              throw new Error('Server returned an invalid response');
            } else {
