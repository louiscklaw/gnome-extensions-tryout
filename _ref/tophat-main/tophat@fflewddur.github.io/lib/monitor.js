...
const [shellMajor, shellMinor] = ShellConfig.PACKAGE_VERSION.split('.').map(s =>
  Number(s),
);
  ...
  class TopHatMeter extends St.BoxLayout {
    ...
    setUsage(usage) {
      ...
      for (let i = 0; i < usage.length; i++) {
        let fillHeight =
          Math.ceil((usage[i] / 100.0) * height) / this.scaleFactor;
        if (isNaN(fillHeight)) {
          fillHeight = 0;
        }
    ...
    computeBarWidth(numBars, width) {
      if (numBars > 8) {
        width *= 0.5; // Reduce bar width by half when there are many bars
      } else if (numBars > 2) {
        width *= 0.75; // Reduce bar width by 3/4 when there are a few bars
      }
      return width;
    }
    ...
    enable_animation() {
      ...
      if (!this.is_realized()) {
        ...
        this.animationTimer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 500, () =>
