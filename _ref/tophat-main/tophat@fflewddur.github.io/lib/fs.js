...
class ProcDiskActivity {
  ...
  readRate() {
    if (this.timePrev > 0) {
      return (
        (this.read - this.readPrev) /
        ((this.time - this.timePrev) / Shared.SECOND_AS_MICROSECONDS)
      );
    }
    return 0;
  }
  ...
  class TopHatFileSystemMonitor extends Monitor.TopHatMonitor {
    _init(configHandler) {
      ...
      let gicon = Gio.icon_new_for_string(
        `${Me.path}/icons/disk-icon-symbolic.svg`,
      );
      ...
      let valueRead = new St.Label({
        text: '0',
        style_class: 'tophat-meter-value-net',
        y_expand: true,
        y_align: Clutter.ActorAlign.END,
      });
      vbox.add_child(valueRead);
      this.valueRead = valueRead;
      let valueWrite = new St.Label({
        text: '0',
        style_class: 'tophat-meter-value-net',
        y_expand: true,
    ...
    _startTimers() {
      ...
      this.history = [];
      let updateInterval = this.computeDetailsUpdateInterval(
        Config.UPDATE_INTERVAL_DISK,
      );
    ...
    _buildMenu() {
      ...
      for (let i = 0; i < Config.N_TOP_PROCESSES; i++) {
        ...
        let usage = [];
        usage.push(
          new St.Label({ text: '', style_class: 'menu-disk-activity' }),
        );
        usage.push(
          new St.Label({ text: '', style_class: 'menu-disk-activity' }),
        );
    ...
    _refreshFS() {
      ...
      mounts.forEach(mountPath => {
        ...
        let fs = new FSUsage(
          mountPath,
          fsu.blocks * fsu.block_size,
          fsu.bfree * fsu.block_size,
        );
        ...
        label = new St.Label({
          text: `${Shared.bytesToHumanString(
            fs.free,
          )} available of ${Shared.bytesToHumanString(fs.size)}`,
    ...
    _refreshIO() {
      ...
      for (let i = 0; i < Config.N_TOP_PROCESSES; i++) {
        ...
        let readActivity = '',
          writeActivity = '';
        if (procList[i].cmd) {
          readActivity = `${Shared.bytesToHumanString(
            procList[i].readRate(),
          )}/s`;
          writeActivity = `${Shared.bytesToHumanString(
            procList[i].writeRate(),
          )}/s`;
        }
      ...
      this.history.push(new DiskActivity(Math.round(read), Math.round(write)));
    ...
    _repaintHistory() {
      ...
      this.historyMaxVal.text = `${Shared.bytesToHumanString(
        max,
        'bytes',
        true,
      )}/s`;
