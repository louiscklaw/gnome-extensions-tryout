  ...
  class TopHatMemMonitor extends Monitor.TopHatMonitor {
    _init(configHandler) {
      ...
      let gicon = Gio.icon_new_for_string(
        `${Me.path}/icons/mem-icon-symbolic.svg`,
      );
    ...
    _readMemInfo() {
      new FileModule.File('/proc/meminfo')
        ...
        .then(lines => {
          ...
          this.menuMemUsage.text = `${(this.memUsage.mem * 100).toFixed(0)}%`;
          this.menuMemSize.text = `${(
            this.memUsage.memUsed / KB_PER_GB
          ).toFixed(1)} GB of ${(this.memUsage.memSize / KB_PER_GB).toFixed(
            1,
          )} GB`;
          ...
          this.menuSwapUsage.text = `${(this.memUsage.swap * 100).toFixed(0)}%`;
          this.menuSwapSize.text = `${(
            this.memUsage.swapUsed / KB_PER_GB
          ).toFixed(1)} GB of ${(this.memUsage.swapSize / KB_PER_GB).toFixed(
            1,
          )} GB`;
        })
        .catch(err => {
